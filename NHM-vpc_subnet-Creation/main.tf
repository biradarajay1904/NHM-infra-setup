...
resource "aws_subnet" "public_subnet" {
  ...
  tags = {
    Name       = "${local.prefix}-PUB-0${count.index + 1}"
    Region     = var.region
    Env        = var.environment
...
resource "aws_subnet" "private_subnet" {
  ...
  tags = {
    Name       = "${local.prefix}-PRI-0${count.index + 1}"
    Region     = var.region
    Env        = var.environment
    Managed_by = var.managed_by
    Resource   = "PRI"
...
resource "aws_eip" "eip" {
  ...
  vpc   = true
  tags = {
    Name       = "${local.prefix}-EIP-0${count.index + 1}"
    Region     = var.region
    Env        = var.environment
    Managed_by = var.managed_by
    Resource   = "EIP"
...
resource "aws_route_table" "private_route_table" {
  ...
  vpc_id = aws_vpc.vpc.id
  tags = {
    Name       = "${local.prefix}-PRI-RT-0${count.index + 1}"
    Region     = var.region
    Env        = var.environment
    Managed_by = var.managed_by
    Resource   = "PRI-RT"
